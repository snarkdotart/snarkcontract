...
library SnarkOfferBidLib {
        ...
        returns (uint256 offerId) 
    {
        ...
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256(abi.encodePacked("saleTypeToToken", _tokenId)), 1); // Offer
    }
    ...
    function cancelOffer(address _storageAddress, uint256 _offerId) public {
        ...
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256(abi.encodePacked("saleTypeToToken", tokenId)), 0); // None
    }

    function getOfferPrice(address _storageAddress, uint256 _offerId) public view returns (uint256 price) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(
    ...
    function setOfferPrice(address _storageAddress, uint256 _offerId, uint256 _price) public {
        SnarkStorage(address(uint160(_storageAddress))).setUint(
    ...
    function getOwnerOfOffer(address _storageAddress, uint256 _offerId) public view returns (address) {
        return SnarkStorage(address(uint160(_storageAddress))).addressStorage(
    ...
    function setOwnerOfOffer(address _storageAddress, uint256 _offerId, address _offerOwner) public {
        SnarkStorage(address(uint160(_storageAddress)))
            .setAddress(keccak256(abi.encodePacked("ownerToOffer", _offerId)), _offerOwner);
    }
    ...
    function getOfferIndexInListForOwner(address _storageAddress, address _offerOwner, uint256 _offerId)
        ...
        returns (uint256)
    {
        SnarkStorage(address(uint160(_storageAddress))).uintStorage(
    ...
    function getTotalNumberOfOffers(address _storageAddress) public view returns (uint256 numberOfOffers) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256("totalNumberOfOffers"));
    }

    function increaseTotalNumberOfOffers(address _storageAddress) public returns (uint256 newAmount) {
        uint256 amount = SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256("totalNumberOfOffers"));
        newAmount = amount.add(1);
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256("totalNumberOfOffers"), newAmount);
    }
    ...
    {
        uint256 amount = SnarkStorage(address(uint160(_storageAddress))).uintStorage(
            keccak256(abi.encodePacked("totalNumberOfOwnerOffers", _offerOwner)));
        newAmount = amount.add(1);
        SnarkStorage(address(uint160(_storageAddress))).setUint(
        ...
        returns (uint256 bidId) 
    {
        ...
        setBidToTokenAndBidOwner(_storageAddress, _bidOwner, _tokenId, bidId);
    ...
    function deleteBid(address _storageAddress, uint256 _bidId) public {
        ...
        setBidToTokenAndBidOwner(_storageAddress, bidOwner, tokenId, 0);
    }
    ...
    function getBidPrice(address _storageAddress, uint256 _bidId) public view returns (uint256) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256(abi.encodePacked("bidPrice", _bidId)));
    }

    function setBidPrice(address _storageAddress, uint256 _bidId, uint256 _price) public {
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256(abi.encodePacked("bidPrice", _bidId)), _price);
    }

    function getBidSaleStatus(address _storageAddress, uint256 _bidId) public view returns (uint256) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256(abi.encodePacked("bidSaleStatus", _bidId)));
    }

    function setBidSaleStatus(address _storageAddress, uint256 _bidId, uint256 _saleStatus) public {
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256(abi.encodePacked("bidSaleStatus", _bidId)), _saleStatus);
    }

    function getMaxBidForToken(address _storageAddress, uint256 _tokenId) public view returns (uint256) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256(abi.encodePacked("maxBidIdForToken", _tokenId)));
    }

    function setMaxBidForToken(address _storageAddress, uint256 _tokenId, uint256 _bidId) public {
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256(abi.encodePacked("maxBidIdForToken", _tokenId)), _bidId);
    }

    function getMaxBidPriceForToken(address _storageAddress, uint256 _tokenId) public view returns (uint256) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256(abi.encodePacked("maxBidPriceForToken", _tokenId)));
    }

    function setMaxBidPriceForToken(address _storageAddress, uint256 _tokenId, uint256 _price) public {
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256(abi.encodePacked("maxBidPriceForToken", _tokenId)), _price);
    }
    ...
    function setOwnerOfBid(address _storageAddress, uint256 _bidId, address _bidOwner) public {
        SnarkStorage(address(uint160(_storageAddress))).setAddress(keccak256(abi.encodePacked("ownerToBid", _bidId)), _bidOwner);
    }

    function getTokenByBid(address _storageAddress, uint256 _bidId) public view returns (uint256) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256(abi.encodePacked("tokenIdToBidId", _bidId)));
    }

    function setTokenToBid(address _storageAddress, uint256 _bidId, uint256 _tokenId) public {
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256(abi.encodePacked("tokenIdToBidId", _bidId)), _tokenId);
    }

    function getTotalNumberOfBids(address _storageAddress) public view returns (uint256) {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256("totalNumberOfBids"));
    }

    function increaseTotalNumberOfBids(address _storageAddress) public returns (uint256 newAmount) {
        uint256 amount = SnarkStorage(address(uint160(_storageAddress))).uintStorage(keccak256("totalNumberOfBids"));
        newAmount = amount.add(1);
        SnarkStorage(address(uint160(_storageAddress))).setUint(keccak256("totalNumberOfBids"), newAmount);
    }
    ...
    function deleteBidFromTokenBidsList(address _storageAddress, uint256 _bidId) public {
        uint256 tokenId = getTokenByBid(_storageAddress, _bidId);
        require(
            getBidActivityForToken(_storageAddress, _bidId, tokenId), 
            "bid is already excluded from the token's list"
        );
    ...
    function getBidActivityForToken(address _storageAddress, uint256 _bidId, uint256 _tokenId)
        ...
        returns (bool)
    {
        return SnarkStorage(address(uint160(_storageAddress))).boolStorage(
    ...
    function setBidActivityForToken(address _storageAddress, uint256 _bidId, uint256 _tokenId, bool _isActive) public {
        SnarkStorage(address(uint160(_storageAddress))).setBool(
    ...
    function deleteBidFromOwnerBidsList(address _storageAddress, uint256 _bidId) public {
        address bidOwner = getOwnerOfBid(_storageAddress, _bidId);
        require(
            getBidActivityForOwner(_storageAddress, _bidId, bidOwner), 
            "bid is already excluded from the owner's list"
        );
    ...
    function getBidForTokenAndBidOwner(address _storageAddress, address _bidOwner, uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        return SnarkStorage(address(uint160(_storageAddress))).uintStorage(
            keccak256(abi.encodePacked("mappingBidToTokenAndBidOwner", _bidOwner, _tokenId))
        );
    }

    function setBidToTokenAndBidOwner(address _storageAddress, address _bidOwner, uint256 _tokenId, uint256 _bidId)
        public 
    {
        SnarkStorage(address(uint160(_storageAddress))).setUint(
            keccak256(abi.encodePacked("mappingBidToTokenAndBidOwner", _bidOwner, _tokenId)),
    ...
    function prepareTokenToGift(address _storageAddress, address _to, uint256 _tokenId) public {
        uint256 saleStatus = SnarkStorage(address(uint160(_storageAddress)))
        ...
        saleStatus = SnarkStorage(address(uint160(_storageAddress)))
            .uintStorage(keccak256(abi.encodePacked("saleTypeToToken", _tokenId)));
        require(
        ...
        if (_bidId != uint256(0)) {
            ...
            if (_saleStatus == 2) { // Active 
                ...
                uint256 currentBalance = SnarkStorage(address(uint160(_storageAddress))).uintStorage(
                ...
                SnarkStorage(address(uint160(_storageAddress))).transferFunds(address(uint160(_to)), _bidPrice);
            }
        }
    }
}
