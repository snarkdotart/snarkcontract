...
contract('SnarkOfferBid', async (accounts) => {
    ...
    it("4. test addBid and deleteBid functions", async () => {
        ...
        retval = await instance_snarkbase.getWithdrawBalance(SnarkStorage.address);
        assert.equal(retval.toNumber(), bidPrice, "error on step 8");
        ...
        retval = await instance_snarkbase.getWithdrawBalance(tokenOwner);
        assert.equal(retval.toNumber(), web3.toWei(0, 'ether'), "error on step 12");

        retval = await instance_snarkbase.getWithdrawBalance(bidOwner);
        assert.equal(retval.toNumber(), 0, "error on step 13");

        retval = await instance_snarkbase.getWithdrawBalance(SnarkStorage.address);
        assert.equal(retval.toNumber(), sumPrice, "error on step 14");

        await instance.acceptBid(2, { from: tokenOwner });

        retval = await instance_snarkbase.getOwnerOfToken(tokenId);
        assert.equal(retval, bidOwner, "error on step 15");
        ...
        assert.equal(balanceOfTokenOwner, highestPrice, "error on step 18");
        ...
        assert.equal(balanceOfBidOwner, bidPrice, "error on step 19");
        ...
        assert.equal(balanceOfContract, 0, "error on step 20");
        ...
        retval = await instance.getTotalNumberOfBids();
        assert.equal(retval.toNumber(), 3, "error on step 21");
        
        retval = await instance.getNumberOfTokenBids(tokenId);
        assert.equal(retval.toNumber(), 1, "error on step 22");

        retval = await instance.getNumberBidsOfOwner(tokenOwner);
        assert.equal(retval.toNumber(), 1, "error on step 23");

        retval = await instance.getNumberBidsOfOwner(bidOwner);
        assert.equal(retval.toNumber(), 2, "error on step 24");
        ...
        retval = await instance.getTotalNumberOfBids();
        assert.equal(retval.toNumber(), 3, "error on step 25");
        
        retval = await instance.getNumberOfTokenBids(tokenId);
        assert.equal(retval.toNumber(), 0, "error on step 26");

        retval = await instance.getNumberBidsOfOwner(tokenOwner);
        assert.equal(retval.toNumber(), 1, "error on step 27");

        retval = await instance.getNumberBidsOfOwner(bidOwner);
        assert.equal(retval.toNumber(), 2, "error on step 28");
